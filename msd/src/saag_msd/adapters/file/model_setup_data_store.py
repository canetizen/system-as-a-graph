...
from saag_msd.model.model_setup_data import document_file_name

#: Directory documents are written to when the deployment configures none.
#: Mounted as a volume so produced files outlive the container and CSM-01 can
#: read them.
DEFAULT_OUTPUT_DIR = Path("/var/lib/saag/msd")


def output_dir(configured: str | os.PathLike[str] | None = None) -> Path:
    """Resolve the directory documents are written to.

    Args:
        configured: Directory the deployment asked for, or None for the default.
