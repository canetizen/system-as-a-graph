...
| Scoring/classification method not yet defined | Score/class marked provisional, per `../../reviews/CDR.md` CDR-14 — same convention as `07`'s CDR-08 caveat. |
...
| Automation Client-facing result format vs. this screen's human-readable display | The two are different renderings of the same result; the actual wire format to the Automation Client is undetermined (`../../reviews/CDR.md` CDR-22), so this doc only specifies the human-facing presentation. |
