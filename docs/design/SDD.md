...
The specification names in Table 2 are the registry's vocabulary, so they, the interface IDs, and the SRS requirements they realize stay one-to-one. Alongside them the framework host publishes specifications of its own, which are not CSCI interfaces but the mechanism by which a CSU reaches the process-wide surfaces it must not own: `saag.platform.api-router-provider`, through which a CSU contributes its endpoints to the CSCI's single external REST surface; `saag.platform.task-provider` with `saag.platform.job-queue`, through which a CSU contributes and defers long-running operations; and `saag.platform.session-authenticator`, through which the host asks whether a request carries an authenticated session.

**Edge authentication.** VAE-01.3 admits only authenticated operators to *the system*, and the system presents one external surface assembled from whichever CSUs are installed. The host therefore makes that decision once, in front of every route it mounts, by asking whoever provides `saag.platform.session-authenticator` — in practice the CSU realizing §3.6.1.2's Session & Authentication Manager, which owns the directory integration. Leaving it to each CSU would put an authentication path in every CSU, make each new CSU one omission away from an open endpoint, and leave the host's own operational endpoints covered by nobody. Two consequences follow deliberately. The decision is *whether there is a session*, never which authorization an operation needs: that stays with the CSU serving the operation, because only it knows. And with no provider registered the host refuses everything but liveness — a CSCI that cannot authenticate anyone is not a CSCI with open access — so a composition without the panel serves nothing, which is the safe direction rather than an omission.

The surface is also cross-origin to the only client with a user interface, which is served from an origin of its own (SDP §5). The host answers a browser only from origins the deployment names; naming none refuses all of them, because answering any would let a page anywhere drive the CSCI with an operator's session.
...
**Uniform CSU structure.** Every CSU is one distribution, built and released on its own, containing exactly one bundle module. The hexagonal boundary the SDP fixes (`api/`, `use_cases/`, `model/`, `ports/`, `adapters/`) is unchanged, and the bundle module's whole content is: build the CSU's wiring when the component is validated, publish the CSU's provided specifications, declare the specifications and configuration the CSU requires, and drop the wiring when the component is invalidated. A CSU's `api/` therefore holds *all* of its inbound adapters, the REST endpoints and the provided-service implementation alike, not only the REST ones.

The bundle module is the only place a CSU names the **framework**: nothing else in a CSU imports it, so the use cases, domain model, ports and adapters could be exercised, and the CSU released, with no framework present. A CSU does name the **specifications** it provides or consumes outside the bundle module — an adapter that implements a provided interface, or one that calls a required one, has to be typed against it — and that is the intended dependency: on the contract, never on the CSU behind it.

Wiring is separated from the component that owns it. A CSU's composition root is an ordinary function taking its configuration as arguments and returning the wired object graph; the bundle module supplies the arguments from its declared properties and holds the result. Composition is therefore testable, and reviewable, without a framework or an environment.
...
This is a constraint on the other CSUs as much as a statement about this one: an operator action on another CSU is an operation on that CSU's internal interface, reached through VAE-01, never an endpoint of its own. A CSU publishing an operator-facing endpoint would put a second surface outside the panel's workflow, outside the requirements' traceability, and outside the one place authentication and authorization are decided. Each CSU's own REST contribution is therefore limited to its liveness probe.

##### 3.6.1.2 Design elements

**Session & Authentication Manager** — authenticates users against a defined LDAP directory service (EXT-IF-06) and restricts access to their authorizations; lets the user select the working project/platform/system version and see the currently effective version. Its verdict on a credential is also what the framework host's edge asks for on every request to the CSCI, for every CSU (§2.3.1), which is how "access to the system" comes to mean the whole system rather than this CSU's endpoints. Traces to VAE-01.3–4.

**Model Setup Data Workflow Manager** — lists Model Setup Data files for the selected project/platform/version and lets the user pick one; starts and monitors the Model Setup Data production process (in progress/successful/failed) and the Core System Model creation process; continuously displays data-source accessibility status; displays missing-data/access/authorization/format/integrity errors from MSD. It also carries what the operator must do to MSD before a run is possible at all — configuring the external data sources, recording the baseline software unit version inventory and its candidate versions, and obtaining or entering the network topology (MSD.6–8, 14–15) — because §3.6.1.1 leaves those no other surface to be performed from; the panel forwards them and reports what MSD answered, interpreting none of it. Traces to VAE-01.5–9, and to VAE-01.2 for the operations it carries on MSD's behalf.
