...
from msd.src.adapters.ansible.network_topology_source import (
    AnsibleNetworkTopologySource,
)
from msd.src.adapters.git.source_code_repository import GitSourceCodeRepository
from msd.src.adapters.rest.package_repository import RestPackageRepository
from msd.src.adapters.rules_config import load_rules
from msd.src.adapters.sql.configuration_management_database import (
...
STANDINS = Path(__file__).resolve().parents[1] / "standins"
...
def git_server(tmp_path: Path) -> Path:
    ...
    """
    unit = STANDINS / "data" / "source_repository" / "bitbucket-a" / "nav_app_1.2.0"
    work = tmp_path / "work"
    work.mkdir()
...
def test_ansible_adapter_reads_machines_and_network_components():
    ...
    adapter = AnsibleNetworkTopologySource(
        _configuration(
            DataSourceType.NETWORK_TOPOLOGY, AccessMethod.ANSIBLE, str(STANDINS / "ansible")
        )
    )
...
_QUERIES = json.loads((STANDINS / "sources.json").read_text(encoding="utf-8"))
_CMDB_PARAMETERS = next(
    source["parameters"]
    for source in _QUERIES["sources"]
