...
from saag_vae_operations_panel.model.session import (
    AuthenticatedUser,
    Authorization,
    Session,
)
...
class JwtTokenService:
    ...
    def __init__(self, clock: ClockPort, lifetime: timedelta, secret: str | None = None) -> None:
        ...
        if not self._secret:
            raise RuntimeError(
                f"{SECRET_SETTING} is not set; the operations panel cannot issue "
                f"session tokens without a signing secret"
            )
        if len(self._secret) < _MINIMUM_SECRET_LENGTH:
            raise RuntimeError(
                f"{SECRET_SETTING} must be at least {_MINIMUM_SECRET_LENGTH} "
